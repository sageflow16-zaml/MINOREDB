    ...
    <div className="p-6 md:p-8 space-y-6 max-w-screen-2xl mx-auto">
      ...
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-primary/10"><SettingsIcon className="h-5 w-5 text-primary-text" /></div>
          ...
          {tabs.map((t) => {
            ...
            return (
              <button key={t.id} role="tab" aria-selected={activeTab === t.id} aria-controls={`panel-${t.id}`}
                ...
                className={cn('flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-medium transition-all text-left', activeTab === t.id ? 'bg-primary/10 text-primary-text' : 'text-muted hover:text-secondary hover:bg-surface')}>
        ...
        <div className="flex lg:hidden gap-1 overflow-x-auto pb-1" role="tablist" aria-label="Settings tabs">
          {tabs.map((t) => {
            ...
            return (
              <button key={t.id} role="tab" aria-selected={activeTab === t.id} aria-controls={`panel-${t.id}`}
                onClick={() => setActiveTab(t.id)}
                className={cn('shrink-0 rounded-lg px-3 py-1.5 text-xs font-medium transition-all flex items-center gap-1', activeTab === t.id ? 'bg-primary/10 text-primary-text' : 'text-muted bg-surface')}>
            ...
            <>
              <SettingsSection title="Profile" desc="Your trading identity and style">
                <div className="flex items-center gap-4 mb-4">
                  <div className="flex h-16 w-16 items-center justify-center rounded-full bg-primary/10 text-xl font-bold text-primary-text">
          ...
          {activeTab === 'security' && (
            <SettingsSection title="Security" desc="Account security is managed by Supabase Auth">
              ...
              <SettingRow label="Sign Out" desc="End this session">
                <Button variant="outline" size="sm" className="text-danger-text border-danger/30" onClick={async () => { await logout(); navigate('/login'); }}>Sign Out</Button>
              </SettingRow>
            </SettingsSection>
          )}
          ...
          {activeTab === 'data' && (
            <SettingsSection title="Data Management" desc="Manage your trading data">
              ...
              <SettingRow label="Clear All Data" desc="Permanently delete all trading data">
                <Button variant="outline" size="sm" className="text-danger-text border-danger/30" onClick={clearAllData}><Trash2 className="h-3.5 w-3.5 mr-1" />Clear Data</Button>
              </SettingRow>
              <SettingRow label="Delete Project" desc="Permanently delete this project and all of its data">
                <Button variant="outline" size="sm" className="text-danger-text border-danger/30" onClick={deleteProject}><Trash2 className="h-3.5 w-3.5 mr-1" />Delete Project</Button>
              </SettingRow>
            </SettingsSection>
          )}
        </div>
