  ...
  return (
    <div className="p-6 md:p-8 space-y-6 max-w-screen-2xl mx-auto">
      ...
      <div className="flex items-center gap-2 rounded-xl border border-warning/20 bg-warning/5 px-4 py-3">
        <SettingsIcon className="h-4 w-4 text-warning shrink-0" />
        <p className="text-xs text-warning">Settings persistence is not yet available. This page shows a preview of the settings layout — no changes are saved.</p>
      </div>
        ...
        <div className="flex-1 space-y-5" role="tabpanel" id={`panel-${activeTab}`}>
              ...
              <SettingsSection title="Profile" desc="Manage your personal information">
                <div className="space-y-4">
                  <div className="flex items-center gap-4 mb-4">
                    <div className="flex h-16 w-16 items-center justify-center rounded-full bg-primary/10 text-xl font-bold text-primary">T</div>
                    <div><p className="text-sm font-medium text-foreground">Trader Name</p><p className="text-xs text-muted">trader@example.com</p><Button variant="ghost" size="sm" className="mt-1 text-2xs">Change Avatar</Button></div>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div><label htmlFor="settings-name" className="text-xs text-muted mb-1 block">Name</label><Input id="settings-name" defaultValue="Trader" className="text-xs" /></div>
                    <div><label htmlFor="settings-email" className="text-xs text-muted mb-1 block">Email</label><Input id="settings-email" defaultValue="trader@example.com" className="text-xs" /></div>
                    <div><label htmlFor="settings-style" className="text-xs text-muted mb-1 block">Trading Style</label><select id="settings-style" className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-xs text-secondary"><option>Swing Trader</option><option>Day Trader</option><option>Scalper</option><option>Position Trader</option></select></div>
                    <div><label htmlFor="settings-exp" className="text-xs text-muted mb-1 block">Experience</label><select id="settings-exp" className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-xs text-secondary"><option>Beginner</option><option>Intermediate</option><option>Advanced</option><option>Professional</option></select></div>
                  </div>
                </div>
              </SettingsSection>
            </>
          ...
          {activeTab === 'workspace' && (
            <SettingsSection title="Workspace" desc="Configure your default workspace settings">
              <SettingRow label="Default Dashboard" desc="Choose your landing page"><select className="rounded-lg border border-border bg-surface px-2 py-1.5 text-xs text-secondary"><option>Dashboard</option><option>Portfolio</option><option>Analytics</option></select></SettingRow>
              <SettingRow label="Default Date Range" desc="Chart date range preset"><select className="rounded-lg border border-border bg-surface px-2 py-1.5 text-xs text-secondary"><option>1 Month</option><option>3 Months</option><option>6 Months</option><option>1 Year</option></select></SettingRow>
              <SettingRow label="Default Currency" desc="Display currency"><select className="rounded-lg border border-border bg-surface px-2 py-1.5 text-xs text-secondary"><option>USD ($)</option><option>EUR (€)</option><option>GBP (£)</option></select></SettingRow>
              <SettingRow label="Trade Timezone" desc="Time zone for all trading data"><select className="rounded-lg border border-border bg-surface px-2 py-1.5 text-xs text-secondary"><option>UTC</option><option>EST</option><option>GMT</option></select></SettingRow>
            </SettingsSection>
          )}
          ...
          {activeTab === 'security' && (
            <SettingsSection title="Security" desc="Manage your account security">
              ...
              <SettingRow label="Session Duration" desc="Auto-logout after inactivity"><select className="rounded-lg border border-border bg-surface px-2 py-1.5 text-xs text-secondary"><option>30 minutes</option><option>1 hour</option><option>4 hours</option><option>8 hours</option></select></SettingRow>
          ...
          {activeTab === 'notifications' && (
            <SettingsSection title="Notifications" desc="Control how you receive updates">
              ...
              <SettingRow label="Daily Digest" desc="Receive a daily trading summary"><select className="rounded-lg border border-border bg-surface px-2 py-1.5 text-xs text-secondary"><option>Enabled</option><option>Disabled</option></select></SettingRow>
            </SettingsSection>
          )}
          ...
          {activeTab === 'appearance' && (
            <SettingsSection title="Appearance" desc="Customize the look and feel">
              ...
              <SettingRow label="Font Size" desc="UI text size"><select className="rounded-lg border border-border bg-surface px-2 py-1.5 text-xs text-secondary"><option>Small</option><option>Medium</option><option>Large</option></select></SettingRow>
              <SettingRow label="Density" desc="Content density"><select className="rounded-lg border border-border bg-surface px-2 py-1.5 text-xs text-secondary"><option>Compact</option><option>Comfortable</option><option>Spacious</option></select></SettingRow>
              <SettingRow label="Animations" desc="UI motion effects"><select className="rounded-lg border border-border bg-surface px-2 py-1.5 text-xs text-secondary"><option>Enabled</option><option>Reduced</option></select></SettingRow>
            </SettingsSection>
          )}
            ...
            <SettingsSection title="API Keys" desc="Manage your API access keys">
              <div className="space-y-3">
                {[
                  ...
                  <div key={k.name} className="flex items-center justify-between rounded-lg bg-surface px-4 py-3">
                    <div><p className="text-sm text-foreground">{k.name}</p><p className="text-xs text-muted font-mono">{k.key}</p><p className="text-3xs text-muted">Created: {k.created}</p></div>
                    <div className="flex gap-2"><Button variant="ghost" size="sm">Copy</Button><Button variant="ghost" size="sm" className="text-danger">Revoke</Button></div>
                  </div>
                ))}
          ...
          {activeTab === 'data' && (
            <SettingsSection title="Data Management" desc="Manage your trading data">
              ...
              <SettingRow label="Clear All Data" desc="Permanently delete all trading data"><Button variant="outline" size="sm" className="text-danger border-danger/30">Clear Data</Button></SettingRow>
            </SettingsSection>
          )}
        </div>
      </div>
