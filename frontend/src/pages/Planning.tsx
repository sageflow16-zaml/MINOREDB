...
export default function PlanningPage() {
  ...
  if (!isLoading && !isError && !dashboard.data) {
    return <EmptyState icon={<Calendar className="h-6 w-6" />} title="No planning data" description="Start planning your trading day." />;
  }
  ...
  if (isLoading) {
    return (<div className="p-6 md:p-8 space-y-6 max-w-screen-2xl mx-auto"><div className="flex items-center justify-between"><div className="space-y-2"><Skeleton className="h-7 w-36" /><Skeleton className="h-4 w-52" /></div></div><div className="grid grid-cols-2 sm:grid-cols-4 gap-3">{Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} className="h-24 rounded-xl" />)}</div><Skeleton className="h-72 rounded-xl" /></div>);
  }
    ...
    <div className="p-6 md:p-8 space-y-6 max-w-screen-2xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.08 }} className="rounded-xl border border-border bg-card p-5">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2"><Calendar className="h-4 w-4 text-muted" /><h3 className="text-sm font-medium text-foreground">{MONTHS[currentMonth.getMonth()]} {currentMonth.getFullYear()}</h3></div>
          ...
          <div className="grid grid-cols-7 gap-1">
            {DAYS.map((d) => <div key={d} className="text-center text-3xs font-medium text-muted py-1.5">{d}</div>)}
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="rounded-xl border border-border bg-card p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-medium text-foreground">{selectedDate}</h3>
          ...
          <div className="space-y-2">
            {dv?.sessions?.map((s: any, i: number) => (
              <div key={i} className={cn('flex items-center justify-between rounded-lg bg-background px-3 py-2', s.is_current && 'border border-primary/30')}>
                <div className="flex items-center gap-2"><div className={cn('h-2 w-2 rounded-full', s.is_current ? 'bg-success animate-pulse' : 'bg-muted')} /><span className="text-xs text-secondary">{s.name}</span></div>
                <span className="text-3xs text-muted">{s.start_time}-{s.end_time}</span>
              </div>
            ))}
            {(dv?.economic_events ?? []).slice(0, 3).map((ev: any, i: number) => (
              <div key={i} className="flex items-center gap-2 rounded-lg bg-background px-3 py-2"><Badge variant={IMPACT_COLORS[ev.impact_level as keyof typeof IMPACT_COLORS] || 'secondary'} size="sm">{ev.impact_level}</Badge><span className="text-xs text-secondary">{ev.event_name}</span><span className="text-3xs text-muted ml-auto">{ev.currency}</span></div>
            ))}
            {!dv?.sessions?.length && !dv?.economic_events?.length && <p className="text-xs text-muted py-6 text-center">No activity for this date</p>}
          </div>
        </motion.div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.12 }} className="rounded-xl border border-border bg-card p-5">
          <div className="flex items-center justify-between mb-4"><h3 className="text-sm font-medium text-foreground">Daily Plan</h3><Button variant="ghost" size="sm" onClick={() => setShowCreatePlan(!showCreatePlan)}>{dv?.plan ? 'Edit' : 'Create'}</Button></div>
          ...
          {dv?.plan ? (
            <div className="space-y-3">
              <div className="flex items-center gap-2"><Badge variant={dv.plan.market_bias === 'bullish' ? 'success' : dv.plan.market_bias === 'bearish' ? 'destructive' : 'default'} size="sm">{dv.plan.market_bias || 'No bias'}</Badge><span className="text-xs text-muted">{(dv.plan.watchlist || []).length} watchlist pairs</span></div>
              {dv.plan.notes && <p className="text-xs text-secondary">{dv.plan.notes}</p>}
            </div>
          ) : <p className="text-xs text-muted py-4 text-center">No plan for today. Create one to start with structure.</p>}
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.14 }} className="rounded-xl border border-border bg-card p-5">
          <div className="flex items-center justify-between mb-4"><h3 className="text-sm font-medium text-foreground">Goals ({goalList.length})</h3><Button variant="ghost" size="sm" onClick={() => setShowCreateGoal(!showCreateGoal)}><Plus className="h-3.5 w-3.5 mr-1" />Add</Button></div>
          ...
          {goalList.length > 0 ? (
            <div className="space-y-2">
              {goalList.slice(0, 5).map((g: Goal) => {
                ...
                return (
                  <div key={g.id} className="rounded-lg bg-background px-3 py-2">
          ...
          ) : <p className="text-xs text-muted py-4 text-center">No goals set. Create one to track progress.</p>}
        </motion.div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.16 }} className="rounded-xl border border-border bg-card p-5">
          <div className="flex items-center justify-between mb-4"><h3 className="text-sm font-medium text-foreground">Checklists</h3><Button variant="ghost" size="sm" onClick={() => setShowCreateChecklist(!showCreateChecklist)}><Plus className="h-3.5 w-3.5 mr-1" />New</Button></div>
          ...
          {templateList.length > 0 ? templateList.map((tmpl: ChecklistTemplate) => (
            <div key={tmpl.id} className="mb-2 rounded-lg bg-background p-3">
              <div className="flex items-center justify-between mb-2"><span className="text-xs font-medium text-foreground">{tmpl.name}</span><Badge variant="outline" size="sm">{tmpl.checklist_type?.replace(/_/g, ' ')}</Badge></div>
              <div className="space-y-1">
                {(tmpl.items ?? []).slice(0, 4).map((it: any, i: number) => (
                  <div key={i} className="flex items-center gap-2"><div className="h-3.5 w-3.5 rounded border border-border" /><span className="text-2xs text-muted">{it.label}</span></div>
                ))}
              </div>
              <Button variant="outline" size="sm" className="mt-2 w-full text-3xs" onClick={() => createChecklistExec.mutate({ template_id: tmpl.id, execution_date: selectedDate, completed_items: (tmpl.items ?? []).map((it: any) => ({ label: it.label, completed: true })) })}><CheckCircle className="h-3 w-3 mr-1" />Complete All</Button>
            </div>
          )) : <p className="text-xs text-muted py-4 text-center">No checklists. Create a pre-market routine.</p>}
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.18 }} className="rounded-xl border border-border bg-card p-5">
          <div className="flex items-center justify-between mb-4"><h3 className="text-sm font-medium text-foreground">Reminders ({reminderList.length})</h3><Button variant="ghost" size="sm" onClick={() => setShowCreateReminder(!showCreateReminder)}><Plus className="h-3.5 w-3.5 mr-1" />Add</Button></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="rounded-xl border border-border bg-card p-5">
          <div className="flex items-center justify-between mb-4"><h3 className="text-sm font-medium text-foreground">Economic Events</h3><Button variant="ghost" size="sm" onClick={() => setShowCreateEconEvent(!showCreateEconEvent)}><Plus className="h-3.5 w-3.5 mr-1" />Add</Button></div>
          ...
          {econEvents.data && (econEvents.data as EconomicEvent[]).length > 0 ? (econEvents.data as EconomicEvent[]).map((ev: EconomicEvent) => (
            <div key={ev.id} className="flex items-center gap-2 rounded-lg bg-background px-3 py-2 mb-1">
              <Badge variant={IMPACT_COLORS[ev.impact_level as keyof typeof IMPACT_COLORS] || 'secondary'} size="sm">{ev.impact_level}</Badge>
              <span className="text-xs text-secondary flex-1">{ev.event_name}</span>
              <span className="text-3xs text-muted">{ev.currency}</span>
              <span className="text-3xs text-muted font-mono">{ev.event_time || '-'}</span>
            </div>
          )) : <p className="text-xs text-muted py-4 text-center">No economic events for this date.</p>}
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.22 }} className="rounded-xl border border-border bg-card p-5">
          <div className="flex items-center justify-between mb-4"><h3 className="text-sm font-medium text-foreground">Post-Market Reviews</h3><Button variant="ghost" size="sm" onClick={() => setShowCreateReview(!showCreateReview)}><Plus className="h-3.5 w-3.5 mr-1" />New</Button></div>
          ...
          {reviewList.length > 0 ? reviewList.slice(0, 5).map((rev: DailyReview) => (
            <div key={rev.id} className="rounded-lg bg-background p-3 mb-2">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-medium text-foreground">{rev.review_date}</span>
              ...
              {rev.daily_summary && <p className="text-xs text-muted">{rev.daily_summary}</p>}
            </div>
          )) : <p className="text-xs text-muted py-4 text-center">No reviews yet. Complete a post-market review.</p>}
        </motion.div>
      </div>
    </div>
  );
...
function CreatePlanFormSmall({ date, existing, onSubmit, onCancel }: { date: string; existing?: any; onSubmit: (data: any) => void; onCancel: () => void }) {
  ...
  return (<div className="rounded-lg bg-background p-3 mb-3 space-y-2">
    <select value={marketBias} onChange={(e) => setMarketBias(e.target.value)} className="w-full rounded border border-border bg-card px-2 py-1 text-xs text-secondary"><option value="">Bias...</option><option value="bullish">Bullish</option><option value="bearish">Bearish</option><option value="neutral">Neutral</option></select>
    <Input value={watchlist} onChange={(e) => setWatchlist(e.target.value)} placeholder="Watchlist (comma-separated)" className="text-xs" />
    <textarea value={notes} onChange={(e) => setNotes(e.target.value)} className="w-full rounded border border-border bg-card px-2 py-1 text-xs text-secondary placeholder-muted min-h-[50px]" placeholder="Notes..." />
...
function CreateGoalFormSmall({ onSubmit, onCancel }: { onSubmit: (data: any) => void; onCancel: () => void }) {
  ...
  return (<div className="rounded-lg bg-background p-3 mb-3 space-y-2">
    ...
    <div className="flex gap-2"><Input type="number" value={target} onChange={(e) => setTarget(e.target.value)} placeholder="Target" className="text-xs flex-1" />
      <select value={priority} onChange={(e) => setPriority(e.target.value)} className="rounded border border-border bg-card px-2 py-1 text-xs text-secondary"><option value="low">Low</option><option value="medium">Med</option><option value="high">High</option></select></div>
...
function CreateEconEventFormSmall({ date, onSubmit, onCancel }: { date: string; onSubmit: (data: any) => void; onCancel: () => void }) {
  ...
  return (<div className="rounded-lg bg-background p-3 mb-3 space-y-2">
    ...
    <div className="flex gap-2"><Input value={currency} onChange={(e) => setCurrency(e.target.value)} placeholder="Currency" className="text-xs flex-1" />
      <select value={impact} onChange={(e) => setImpact(e.target.value)} className="rounded border border-border bg-card px-2 py-1 text-xs text-secondary"><option value="low">Low</option><option value="medium">Med</option><option value="high">High</option></select>
...
function CreateChecklistFormSmall({ onSubmit, onCancel }: { onSubmit: (data: any) => void; onCancel: () => void }) {
  ...
  return (<div className="rounded-lg bg-background p-3 mb-3 space-y-2">
    ...
    <textarea value={items} onChange={(e) => setItems(e.target.value)} className="w-full rounded border border-border bg-card px-2 py-1 text-xs text-secondary placeholder-muted min-h-[60px]" placeholder="One item per line" />
...
function CreateReviewFormSmall({ date, onSubmit, onCancel }: { date: string; onSubmit: (data: any) => void; onCancel: () => void }) {
  ...
  return (<div className="rounded-lg bg-background p-3 mb-3 space-y-2">
    <textarea value={summary} onChange={(e) => setSummary(e.target.value)} className="w-full rounded border border-border bg-card px-2 py-1 text-xs text-secondary placeholder-muted min-h-[60px]" placeholder="Daily summary..." />
