  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <Card>
          <CardContent className="py-3">
            <div className="flex items-center gap-2 mb-1">
              ...
              <p className="text-3xs text-muted-foreground">Status</p>
            </div>
        ...
        <Card>
          <CardContent className="py-3">
            <p className="text-3xs text-muted-foreground mb-1">Account</p>
            <p className="text-sm font-bold text-foreground">{data?.account ?? '--'}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="py-3">
            <p className="text-3xs text-muted-foreground mb-1">Total Trades</p>
            <p className="text-sm font-bold text-foreground">{data?.total_trades ?? 0}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="py-3">
            <p className="text-3xs text-muted-foreground mb-1">Total Synced</p>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div className="flex flex-col gap-1">
              <label className="text-3xs font-medium text-muted-foreground">Account</label>
              <Input placeholder="12345678" value={account} onChange={(e) => setAccount(e.target.value)} disabled={data?.connected} />
            </div>
            <div className="flex flex-col gap-1">
              <label className="text-3xs font-medium text-muted-foreground">Server</label>
              <Input placeholder="MetaQuotes-Demo" value={server} onChange={(e) => setServer(e.target.value)} disabled={data?.connected} />
            </div>
            <div className="flex flex-col gap-1">
              <label className="text-3xs font-medium text-muted-foreground">Terminal Path</label>
              ...
              <tbody className="divide-y divide-border">
                {logData.map((log) => (
                  <tr key={log.id} className="hover:bg-muted/20">
                    <td className="px-4 py-2.5 font-mono text-3xs text-foreground">{log.trade_ticket}</td>
