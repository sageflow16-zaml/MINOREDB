# M3 - Complete proposed decision

| Provenance field | Value |
|---|---|
| Artifact ID | M3-005 |
| Authority / Decision ID | M3 |
| Artifact Type | Complete proposed decision |
...
9. **A single empirical result does not conclusively prove or disprove a hypothesis.** One favorable result constitutes supporting evidence, not proof. One unfavorable result constitutes contradicting evidence, not disproof. Evidence accumulates and is weighted; it is not binary.
...
11. **Evidence for a hypothesis based on a substantive departure from the current interpretation does not automatically revise that interpretation.** Strong empirical results for a departure-based hypothesis may create a mandatory or permissive trigger under A7, but the interpretation is revised only through the A6/A7 process, not automatically by M3 evidence alone.
...
17. **M3 does not determine trading-model admission.** Evidence strength assessment informs, but does not itself constitute, the decision of whether a finding is admitted into the trading model. That determination belongs to a later methodology stage (D1 — Trading-Model Admission Criteria, per the original decision decomposition).

18. **M3 does not itself trigger interpretation reconsideration.** Empirical results are inputs that may satisfy A7's trigger conditions (mandatory or permissive) when they bear materially on the documented interpretation foundation. M3 does not modify A7's trigger framework or bypass it.
...
- **Trading-Model Admission Criteria** (the stage governing which findings are reliable enough to influence the user's trading model, per Founding Definition Core Problem Q7) becomes possible once evidence strength can be meaningfully assessed. M3 provides the evidentiary vocabulary and assessment principles that this later stage will draw upon.
...
| Formal criteria for trading-model admission (evidence-strength thresholds for practical adoption) | DEFER TO Trading-Model Admission Criteria decision |
...
| How conflicting adequate studies are eventually reconciled, if ever, at the trading-model level | DEFER TO Trading-Model Admission Criteria decision |
...
**The Trading-Model Admission Criteria decision is now unblocked.**
