# A2-003 - Corrected/ratification block
...
Extractable claim	A source claim that is sufficiently self-contained to be recorded and later attributed independently
Claim boundary	The minimum unit of source content that constitutes one complete, independently assertable proposition
...
Claim boundary: Each independently assertable proposition is a separate extractable claim; they must not be fused into one
...
Extract: Yes — as two separate claims with shared context
Claim 3a: In the London session, price will often seek liquidity above the previous day's high
Claim 3b: This move (liquidity sweep above prior high) creates conditions for a reversal
Claim boundary: Two independently assertable propositions; the causal relationship between them is preserved in shared context, not by merging them
Preserve: All three sentences as source context; the third sentence ("You want to be positioned...") is procedural instruction, not a general assertion about price behavior
Context required: Session context (London) is load-bearing for both claims; the stated causal relationship between the two claims is recorded as shared context
Outside the claims: What "often" means quantitatively; whether "conditions" implies certainty; researcher trading intention
...
Outside the claim: Any researcher generalization such as "therefore displacement always requires a close above the swept level" — this remains outside the extracted source claim
...
Claim boundary: The full three-step sequence as one unit; the steps are not independently assertable without the sequence
...
Outside the claim: Whether the procedure is effective; researcher judgment about what "displacement" means
...
Outside the claim: Any researcher resolution of whether "market makers" refers to institutional participants, a metaphor, or a mechanical process — this remains outside the extracted source claim
...
Outside the claim: Any researcher completion of the gap ("I think the source meant 'reverse'") — this remains outside the extracted source claim
...
Outside the claim: Any researcher synthesis noting they agree or disagree
...
Not extractable as source claim: "he means that price is mechanically driven to stop-loss clusters" — this remains outside the extracted source claim
Claim boundary: The quoted or paraphrased source assertion only; the interpretive gloss is outside the boundary
Preserve: Source assertion in source terms
...
Draw claim boundaries at the level of independently assertable propositions
...
An extractable claim is the minimum source content that constitutes one independently assertable proposition. A passage containing multiple independent propositions must yield multiple extracted claims.

Extraction preserves what the source said, not what the researcher believes the source meant. Meaning added by the researcher remains outside the extracted source claim.
...
Extraction errors must not be silently corrected. The distinction between immutable attributable source content, the researcher's extraction record, and an extraction error must be preserved. The mechanism for correcting extraction errors is unresolved methodology content.
...
The atomic unit is one independently assertable proposition. If removing one part of a passage leaves the other parts still assertable, they are separate claims.
...
Propositions that are causally or conditionally related but each independently assertable must be extracted as separate claims. Their stated causal or conditional relationship is preserved in shared context, not by merging them into one claim.
...
Procedural instructions are extracted as unified sequences. Steps in a procedure are not independently assertable without the sequence; they constitute one claim.

Rhetorical and motivational statements are not extracted. They contain no independently assertable proposition about price, markets, or trading mechanics.

Implied meaning remains outside the extraction boundary. If a proposition is implied but not stated, it remains outside the extracted source claim.
...
Mechanism for correcting extraction errors	Methodology content — not blocking A3
...
Corrections applied:
Replaced "independently evaluable proposition" with "independently assertable proposition" throughout — extraction criterion does not require empirical testability
Added boundary rule for dependent propositions that are each independently assertable — extract as separate claims with shared context preserving stated relationship; corrected Test 3 accordingly
Reduced Rule 4 to minimum semantic requirement — extraction errors must not be silently corrected; removed premature correction mechanism
Removed premature forward assignment of implied meaning to A6 — implied meaning remains outside the extracted source claim without specifying downstream handling
No contradiction with Founding Definition v0.1, Decision D1, or Decision A1 detected
Open methodology questions: 3 (non-linguistic extraction; error correction mechanism; edge-case paraphrase fidelity) — none block A3
12. Stop Statement
Decision A2 is ratified.

A3 — Claim Classification is now unblocked but has not begun.




Apply the four audited corrections to Decision A2 and produce the final ratified decision.
