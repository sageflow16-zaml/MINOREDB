...
class BinanceProvider(BrokerProviderBase):
    def __init__(self):
    ...
    @property
    def provider_name(self) -> str: return "binance"
    @property
    def display_name(self) -> str: return "Binance"
    @property
    def icon(self) -> str: return "binance"
    @property
    ...
    async def connect(self, credentials: dict[str, Any], config: dict[str, Any] | None = None) -> bool:
        ...
        try:
            logger.info("Binance: Connecting with API key %s...", credentials.get("api_key", "")[:8])
    ...
    async def get_accounts(self) -> list[ProviderAccount]:
        return [ProviderAccount(external_id="binance-1", name="Binance Spot", account_type="spot", currency="USDT", balance=5000.0, equity=5020.0)]

    async def get_balance(self, account_id: str) -> float: return 5000.0
    async def get_equity(self, account_id: str) -> float: return 5020.0
    ...
    async def check_health(self) -> ProviderHealth:
        return ProviderHealth(is_reachable=self._connected, latency_ms=35.0)
