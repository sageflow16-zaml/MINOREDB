...
class KrakenProvider(BrokerProviderBase):
    def __init__(self):
    ...
    @property
    def provider_name(self) -> str: return "kraken"
    @property
    def display_name(self) -> str: return "Kraken"
    @property
    def icon(self) -> str: return "kraken"
    @property
    ...
    async def connect(self, credentials: dict[str, Any], config: dict[str, Any] | None = None) -> bool:
        ...
        try:
            logger.info("Kraken: Connecting with API key %s...", credentials.get("api_key", "")[:8])
    ...
    async def get_accounts(self) -> list[ProviderAccount]:
        return [ProviderAccount(external_id="kraken-1", name="Kraken Main", account_type="spot", currency="USD", balance=12000.0, equity=12030.0)]

    async def get_balance(self, account_id: str) -> float: return 12000.0
    async def get_equity(self, account_id: str) -> float: return 12030.0
    ...
    async def check_health(self) -> ProviderHealth:
        return ProviderHealth(is_reachable=self._connected, latency_ms=28.0)
