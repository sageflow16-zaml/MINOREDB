...
def read_triggers(project_id: UUID, project: Project = Depends(get_project_or_404), skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
...
def read_trigger(project_id: UUID, project: Project = Depends(get_project_or_404), id: UUID = ..., db: Session = Depends(get_db)):
...
def create_trigger(project_id: UUID, project: Project = Depends(get_project_or_404), obj_in: ReconsiderationTriggerCreate = ..., db: Session = Depends(get_db)):
    return crud.create(db, project_id=project_id, obj_in=obj_in)

@router.put("/{id}", response_model=ReconsiderationTriggerRead)
def update_trigger(project_id: UUID, project: Project = Depends(get_project_or_404), id: UUID = ..., obj_in: ReconsiderationTriggerUpdate = ..., db: Session = Depends(get_db)):
    db_obj = crud.get(db, id=id, project_id=project_id)
    if not db_obj:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Reconsideration trigger not found")
...
def delete_trigger(project_id: UUID, project: Project = Depends(get_project_or_404), id: UUID = ..., db: Session = Depends(get_db)):
