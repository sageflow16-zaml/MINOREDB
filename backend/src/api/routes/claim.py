...
def read_claims(project_id: UUID, project: Project = Depends(get_project_or_404), skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    return crud.get_multi(db, project_id=project_id, skip=skip, limit=limit)

@router.get("/{id}", response_model=ClaimRead)
def read_claim(project_id: UUID, project: Project = Depends(get_project_or_404), id: UUID = ..., db: Session = Depends(get_db)):
...
def create_claim(project_id: UUID, project: Project = Depends(get_project_or_404), obj_in: ClaimCreate = ..., db: Session = Depends(get_db)):
    return crud.create(db, project_id=project_id, obj_in=obj_in)

@router.put("/{id}", response_model=ClaimRead)
def update_claim(project_id: UUID, project: Project = Depends(get_project_or_404), id: UUID = ..., obj_in: ClaimUpdate = ..., db: Session = Depends(get_db)):
    db_obj = crud.get(db, id=id, project_id=project_id)
    if not db_obj:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Claim not found")
...
def delete_claim(project_id: UUID, project: Project = Depends(get_project_or_404), id: UUID = ..., db: Session = Depends(get_db)):
    if not crud.remove(db, id=id, project_id=project_id):
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Claim not found")
    return None

@router.post("/{claim_id}/extract-concepts")
def extract_concepts(project_id: UUID, project: Project = Depends(get_project_or_404), claim_id: UUID = ..., db: Session = Depends(get_db)):
...
def interpret_claim(project_id: UUID, project: Project = Depends(get_project_or_404), claim_id: UUID = ..., db: Session = Depends(get_db)):
...
def get_claim_graph(project_id: UUID, project: Project = Depends(get_project_or_404), claim_id: UUID = ..., db: Session = Depends(get_db)):
